...
1. In a text editor create a new file `terraform.tfvars` to define the following parameters for your cluster:

    {% include_cached copy-clipboard.html %}
    ~~~
    cluster_name = # fill me in!
    sql_user_name = # fill me in!
    sql_user_password = # fill me in!
    cloud_provider = # fill me in!
    cloud_provider_regions = # fill me in!
    cluster_node_count = # fill me in!
    storage_gib = # fill me in!
    machine_type = # fill me in!
    allow_list_name = # fill me in!
    cidr_ip = # fill me in!
    cidr_mask = # fill me in!
    ...
    Parameters:
      ...
      - `cloud_provider`: the cloud infrastructure provider. Possible values are `GCP`, `AWS`, `AZURE`. Support for Azure: in limited access. Refer to [{{ site.data.products.dedicated }} on Azure](cockroachdb-dedicated-on-azure.html).
      ...
      - `allow_list_name`: the name for the [IP allow list](network-authorization.html#ip-allowlisting). Use a descriptive name to identify the IP allow list.
