...
These sections show how to scale a {{ site.data.products.dedicated }} cluster vertically by adding or removing nodes or horizontally by changing each node's storage and compute resources.
...
{{site.data.alerts.callout_info}}
During [limited access](/docs/{{site.versions["stable"]}}/cockroachdb-feature-availability.html), compute on {{ site.data.products.dedicated }} clusters on Azure cannot be scaled. Refer to [{{ site.data.products.dedicated }} on Azure](cockroachdb-dedicated-on-azure.html).
{{site.data.alerts.end}}
